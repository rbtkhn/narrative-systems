...
VOICE_ROOT = NG_ROOT / "voices" / "baud"
STRATEGY_ROOT = Path(r"C:\dev\strategy-codex")
INDEX_PATH = STRATEGY_ROOT / "statecraft" / "voices" / "baud" / "baud-index.md"
...
def is_baud_source(source_path: Path) -> bool:
    ...
    joined = "\n".join(markers).lower()
    if "jacques baud" in joined or "jacques-baud" in joined or "baud" in joined:
        return True

    if "threads:" in text.lower():
        frontmatter_block = text.split("---", 2)[1].lower() if text.startswith("---\n") and text.count("---") >= 2 else ""
        if re.search(r"^\s*-\s*baud\s*$", frontmatter_block, flags=re.MULTILINE):
            return True
    return False
...
def infer_source_class(frontmatter: dict[str, str], host_slug: str) -> str:
    ...
    if source_form == "interview":
        return "host-pressure test" if host_slug != "baud" else "authored interview"
...
def ensure_existing_rows_tagged(manifest: dict, target_local_paths: set[str]) -> int:
    ...
    for row in manifest.get("sources", []):
        ...
        if "baud" not in voice_slugs:
            voice_slugs.append("baud")
            updated += 1
    return updated
...
def cleanup_out_of_scope_rows(manifest: dict, target_local_paths: set[str]) -> tuple[int, int]:
    ...
    for row in manifest.get("sources", []):
        ...
        if "baud" not in voice_slugs or local_path in target_local_paths:
        ...
        row["voice_slugs"] = [slug for slug in voice_slugs if slug != "baud"]
        baud_tags_removed += 1
        retained_rows.append(row)

    manifest["sources"] = retained_rows
    return rows_removed, baud_tags_removed
...
def build_voice_files(source_basis_suffix: str) -> None:
    all_rows = [row for row in load_manifest().get("sources", []) if "baud" in row.get("voice_slugs", [])]
    ...
    readme = f"""# Voice Record: Jacques Baud
...
| Source basis | `strategy-codex/statecraft/voices/baud/baud-index.md`{source_basis_suffix} |
...
"""
    ...
    source_index = f"""# Jacques Baud Source Index
...
def main() -> None:
    ...
    rows_removed, baud_tags_removed = cleanup_out_of_scope_rows(manifest, target_local_paths)
    ...
    print(f"Removed out-of-scope baud tags: {baud_tags_removed}")
