...
VOICE_ROOT = NG_ROOT / "voices" / "hoh"
STRATEGY_ROOT = Path(r"C:\dev\strategy-codex")
INDEX_PATH = STRATEGY_ROOT / "statecraft" / "voices" / "hoh" / "hoh-index.md"
...
def is_hoh_source(source_path: Path) -> bool:
    ...
    joined = "\n".join(markers).lower()
    if "matt hoh" in joined or "matthew hoh" in joined or "hoh" in joined:
        return True

    if "threads:" in text.lower():
        frontmatter_block = text.split("---", 2)[1].lower() if text.startswith("---\n") and text.count("---") >= 2 else ""
        if re.search(r"^\s*-\s*hoh\s*$", frontmatter_block, flags=re.MULTILINE):
            return True
    return False
...
def infer_source_class(frontmatter: dict[str, str], host_slug: str) -> str:
    ...
    if source_form == "interview":
        return "host-pressure test" if host_slug != "hoh" else "authored interview"
...
def ensure_existing_rows_tagged(manifest: dict, target_local_paths: set[str]) -> int:
    ...
    for row in manifest.get("sources", []):
        ...
        if "hoh" not in voice_slugs:
            voice_slugs.append("hoh")
            updated += 1
    return updated
...
def cleanup_out_of_scope_rows(manifest: dict, target_local_paths: set[str]) -> tuple[int, int]:
    ...
    for row in manifest.get("sources", []):
        ...
        if "hoh" not in voice_slugs or local_path in target_local_paths:
        ...
        row["voice_slugs"] = [slug for slug in voice_slugs if slug != "hoh"]
        hoh_tags_removed += 1
        retained_rows.append(row)

    manifest["sources"] = retained_rows
    return rows_removed, hoh_tags_removed
...
def build_voice_files(source_basis_suffix: str) -> None:
    all_rows = [row for row in load_manifest().get("sources", []) if "hoh" in row.get("voice_slugs", [])]
    ...
    readme = f"""# Voice Record: Matthew Hoh
...
| Source basis | `strategy-codex/statecraft/voices/hoh/hoh-index.md`{source_basis_suffix} |
...
"""
    ...
    source_index = f"""# Matthew Hoh Source Index
...
def main() -> None:
    ...
    rows_removed, hoh_tags_removed = cleanup_out_of_scope_rows(manifest, target_local_paths)
    ...
    print(f"Removed out-of-scope hoh tags: {hoh_tags_removed}")
