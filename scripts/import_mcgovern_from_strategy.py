...
VOICE_ROOT = NG_ROOT / "voices" / "mcgovern"
STRATEGY_ROOT = Path(r"C:\dev\strategy-codex")
INDEX_PATH = STRATEGY_ROOT / "statecraft" / "voices" / "mcgovern" / "mcgovern-index.md"
...
def parse_index_paths() -> list[Path]:
    ...
    for raw in raw_paths:
        ...
        if is_mcgovern_source(source_path):
...
def is_mcgovern_source(source_path: Path) -> bool:
...
def infer_source_class(frontmatter: dict[str, str], host_slug: str) -> str:
    ...
    if source_form == "interview":
        return "host-pressure test" if host_slug != "mcgovern" else "authored interview"
...
def ensure_existing_rows_tagged(manifest: dict, target_local_paths: set[str]) -> int:
    ...
    for row in manifest.get("sources", []):
        ...
        if "mcgovern" not in voice_slugs:
            voice_slugs.append("mcgovern")
            updated += 1
    return updated
...
def cleanup_out_of_scope_rows(manifest: dict, target_local_paths: set[str]) -> tuple[int, int]:
    retained_rows = []
    rows_removed = 0
    mcgovern_tags_removed = 0

    for row in manifest.get("sources", []):
        local_path = row.get("local_path")
        voice_slugs = row.get("voice_slugs", [])
        if "mcgovern" not in voice_slugs or local_path in target_local_paths:
        ...
        row["voice_slugs"] = [slug for slug in voice_slugs if slug != "mcgovern"]
        mcgovern_tags_removed += 1
        retained_rows.append(row)

    manifest["sources"] = retained_rows
    return rows_removed, mcgovern_tags_removed
...
def build_voice_files(source_basis_suffix: str) -> None:
    all_rows = [row for row in load_manifest().get("sources", []) if "mcgovern" in row.get("voice_slugs", [])]
    ...
    readme = f"""# Voice Record: Ray McGovern
...
| Source basis | `strategy-codex/statecraft/voices/mcgovern/mcgovern-index.md`{source_basis_suffix} |
...
"""
    ...
    source_index = f"""# Ray McGovern Source Index
...
def main() -> None:
    ...
    rows_removed, mcgovern_tags_removed = cleanup_out_of_scope_rows(manifest, target_local_paths)
    ...
    print(f"Removed out-of-scope mcgovern tags: {mcgovern_tags_removed}")
