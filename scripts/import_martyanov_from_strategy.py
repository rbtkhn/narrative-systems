...
VOICE_ROOT = NG_ROOT / "voices" / "martyanov"
STRATEGY_ROOT = Path(r"C:\dev\strategy-codex")
INDEX_PATH = STRATEGY_ROOT / "statecraft" / "voices" / "martyanov" / "martyanov-index.md"
...
def parse_index_paths() -> list[Path]:
    ...
    for raw in raw_paths:
        ...
        if is_martyanov_source(source_path):
...
def is_martyanov_source(source_path: Path) -> bool:
    ...
    joined = "\n".join(markers).lower()
    if "andrei martyanov" in joined or "martyanov" in joined:
        return True

    if "threads:" in text.lower():
        frontmatter_block = text.split("---", 2)[1].lower() if text.startswith("---\n") and text.count("---") >= 2 else ""
        if re.search(r"^\s*-\s*martyanov\s*$", frontmatter_block, flags=re.MULTILINE):
            return True
    return False
...
def infer_source_class(frontmatter: dict[str, str], host_slug: str) -> str:
    ...
    if source_form == "interview":
        return "host-pressure test" if host_slug != "martyanov" else "authored interview"
...
def ensure_existing_rows_tagged(manifest: dict, target_local_paths: set[str]) -> int:
    ...
    for row in manifest.get("sources", []):
        ...
        if "martyanov" not in voice_slugs:
            voice_slugs.append("martyanov")
            updated += 1
    return updated
...
def build_voice_files(source_basis_suffix: str) -> None:
    all_rows = [row for row in load_manifest().get("sources", []) if "martyanov" in row.get("voice_slugs", [])]
    ...
    readme = f"""# Voice Record: Andrei Martyanov
...
| Source basis | `strategy-codex/statecraft/voices/martyanov/martyanov-index.md`{source_basis_suffix} |
...
"""
    ...
    source_index = f"""# Andrei Martyanov Source Index
...
"""
    ...
    profile = """# Andrei Martyanov Profile
...
def main() -> None:
    ...
    rows_removed, martyanov_tags_removed = cleanup_out_of_scope_rows(manifest, target_local_paths)
    ...
    print(f"Removed out-of-scope martyanov tags: {martyanov_tags_removed}")
