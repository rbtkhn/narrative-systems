...
VOICE_ROOT = NG_ROOT / "voices" / "blumenthal"
STRATEGY_ROOT = Path(r"C:\dev\strategy-codex")
INDEX_PATH = STRATEGY_ROOT / "statecraft" / "voices" / "blumenthal" / "blumenthal-index.md"
...
def parse_index_paths() -> list[Path]:
    ...
    for raw in raw_paths:
        ...
        if is_blumenthal_source(source_path):
...
def is_blumenthal_source(source_path: Path) -> bool:
    ...
    joined = "\n".join(markers).lower()
    if "max blumenthal" in joined or "max-blumenthal" in joined or "blumenthal" in joined:
        return True

    if "threads:" in text.lower():
        frontmatter_block = text.split("---", 2)[1].lower() if text.startswith("---\n") and text.count("---") >= 2 else ""
        if re.search(r"^\s*-\s*blumenthal\s*$", frontmatter_block, flags=re.MULTILINE):
            return True
    return False
...
def infer_host_slug(path: Path, frontmatter: dict[str, str]) -> str:
    if path.name == "source-blumenthal-israel-defeat-zionist-power-2026-04-21.md":
...
def infer_source_class(frontmatter: dict[str, str], host_slug: str) -> str:
    ...
    if source_form == "interview":
        return "host-pressure test" if host_slug != "blumenthal" else "authored interview"
...
def ensure_existing_rows_tagged(manifest: dict, target_local_paths: set[str]) -> int:
    ...
    for row in manifest.get("sources", []):
        ...
        if "blumenthal" not in voice_slugs:
            voice_slugs.append("blumenthal")
            updated += 1
    return updated
...
def build_voice_files(source_basis_suffix: str) -> None:
    all_rows = [row for row in load_manifest().get("sources", []) if "blumenthal" in row.get("voice_slugs", [])]
    ...
    readme = f"""# Voice Record: Max Blumenthal
...
| Source basis | `strategy-codex/statecraft/voices/blumenthal/blumenthal-index.md`{source_basis_suffix} |
...
"""
    ...
    source_index = f"""# Max Blumenthal Source Index
...
"""
    ...
    profile = """# Max Blumenthal Profile
...
def main() -> None:
    ...
    rows_removed, blumenthal_tags_removed = cleanup_out_of_scope_rows(manifest, target_local_paths)
    ...
    print(f"Removed out-of-scope blumenthal tags: {blumenthal_tags_removed}")
