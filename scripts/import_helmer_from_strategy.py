...
VOICE_ROOT = NG_ROOT / "voices" / "helmer"
STRATEGY_ROOT = Path(r"C:\dev\strategy-codex")
INDEX_PATH = STRATEGY_ROOT / "statecraft" / "voices" / "helmer" / "helmer-index.md"
...
HOST_SLUG_MAP = {
    ...
    "reason-resist": "reason-resist",
    "lascaris": "reason-resist",
    "helmer": "helmer",
}


EXTRA_LOCAL_PATHS = {
    "narrative-geopolitics/archive/sources/2026-07-07/source-dialogue-works-john-helmer-trump-signs-nato-escalation-putins-demilitarization-order-to-poland-2026-07-07.md",
}
...
def parse_index_paths() -> list[Path]:
    ...
    for raw in raw_paths:
        ...
        if is_helmer_source(source_path):
...
def is_helmer_source(source_path: Path) -> bool:
    ...
    if "threads:" in text.lower():
        ...
        if re.search(r"^\s*-\s*helmer\s*$", frontmatter_block, flags=re.MULTILINE):
            return True
    return False
...
def infer_source_class(frontmatter: dict[str, str], host_slug: str) -> str:
    ...
    if source_form == "interview":
        return "host-pressure test" if host_slug != "helmer" else "authored interview"
...
def collect_target_local_paths(source_paths: list[Path]) -> set[str]:
    paths = {build_row_from_source_path(source_path)["local_path"] for source_path in source_paths}
    paths.update(EXTRA_LOCAL_PATHS)
    return paths
...
def ensure_existing_rows_tagged(manifest: dict, target_local_paths: set[str]) -> int:
    ...
    for row in manifest.get("sources", []):
        ...
        if "helmer" not in voice_slugs:
            voice_slugs.append("helmer")
            updated += 1
    return updated
...
def cleanup_out_of_scope_rows(manifest: dict, target_local_paths: set[str]) -> tuple[int, int]:
    retained_rows = []
    rows_removed = 0
    helmer_tags_removed = 0

    for row in manifest.get("sources", []):
        local_path = row.get("local_path")
        voice_slugs = row.get("voice_slugs", [])
        if "helmer" not in voice_slugs or local_path in target_local_paths:
        ...
        row["voice_slugs"] = [slug for slug in voice_slugs if slug != "helmer"]
        helmer_tags_removed += 1
        retained_rows.append(row)

    manifest["sources"] = retained_rows
    return rows_removed, helmer_tags_removed
...
def build_voice_files(source_basis_suffix: str) -> None:
    all_rows = [row for row in load_manifest().get("sources", []) if "helmer" in row.get("voice_slugs", [])]
    ...
    readme = f"""# Voice Record: John Helmer
...
| Source basis | `strategy-codex/statecraft/voices/helmer/helmer-index.md`{source_basis_suffix} |
...
"""
    ...
    source_index = f"""# John Helmer Source Index
...
def main() -> None:
    ...
    rows_removed, helmer_tags_removed = cleanup_out_of_scope_rows(manifest, target_local_paths)
    ...
    print(f"Removed out-of-scope helmer tags: {helmer_tags_removed}")
