...
VOICE_ROOT = NG_ROOT / "voices" / "jermy"
STRATEGY_ROOT = Path(r"C:\dev\strategy-codex")
INDEX_PATH = STRATEGY_ROOT / "statecraft" / "voices" / "jermy" / "jermy-index.md"
...
def parse_index_paths() -> list[Path]:
    ...
    for raw in raw_paths:
        ...
        if is_jermy_source(source_path):
...
def is_jermy_source(source_path: Path) -> bool:
    ...
    joined = "\n".join(markers).lower()
    if "steve jermy" in joined or "commodore jermy" in joined or "jermy" in joined:
        return True

    if "threads:" in text.lower():
        frontmatter_block = text.split("---", 2)[1].lower() if text.startswith("---\n") and text.count("---") >= 2 else ""
        if re.search(r"^\s*-\s*jermy\s*$", frontmatter_block, flags=re.MULTILINE):
            return True
    return False
...
def infer_source_class(frontmatter: dict[str, str], host_slug: str) -> str:
    ...
    if source_form == "interview":
        return "host-pressure test" if host_slug != "jermy" else "authored interview"
...
def ensure_existing_rows_tagged(manifest: dict, target_local_paths: set[str]) -> int:
    ...
    for row in manifest.get("sources", []):
        ...
        if "jermy" not in voice_slugs:
            voice_slugs.append("jermy")
            updated += 1
    return updated
...
def build_voice_files(source_basis_suffix: str) -> None:
    all_rows = [row for row in load_manifest().get("sources", []) if "jermy" in row.get("voice_slugs", [])]
    ...
    readme = f"""# Voice Record: Steve Jermy
...
| Source basis | `strategy-codex/statecraft/voices/jermy/jermy-index.md`{source_basis_suffix} |
...
"""
    ...
    source_index = f"""# Steve Jermy Source Index
...
def main() -> None:
    ...
    rows_removed, jermy_tags_removed = cleanup_out_of_scope_rows(manifest, target_local_paths)
    ...
    print(f"Removed out-of-scope jermy tags: {jermy_tags_removed}")
