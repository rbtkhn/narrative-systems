...
VOICE_ROOT = NG_ROOT / "voices" / "matlock"
STRATEGY_ROOT = Path(r"C:\dev\strategy-codex")
INDEX_PATH = STRATEGY_ROOT / "statecraft" / "voices" / "matlock" / "matlock-index.md"
...
def parse_index_paths() -> list[Path]:
    ...
    for raw in raw_paths:
        ...
        if is_matlock_source(source_path):
...
def is_matlock_source(source_path: Path) -> bool:
...
def infer_source_class(frontmatter: dict[str, str], host_slug: str) -> str:
    ...
    if source_form == "interview" and host_slug != "matlock":
        return "host-pressure test"
    if source_form:
        return source_form
    if host_slug != "matlock":
        return "host-pressure test"
...
def ensure_existing_rows_tagged(manifest: dict, target_local_paths: set[str]) -> int:
    ...
    for row in manifest.get("sources", []):
        ...
        if "matlock" not in voice_slugs:
            voice_slugs.append("matlock")
            updated += 1
    return updated
    ...
    for row in manifest.get("sources", []):
        ...
        for key in ("date", "voice_index_path", "upstream_path", "host_slug", "source_class", "modality", "import_status"):
            if row.get(key) != canonical[key]:
                ...
                changed = True
        if "matlock" not in row.setdefault("voice_slugs", []):
            row["voice_slugs"].append("matlock")
            changed = True
        if changed:
            updated += 1
    return updated
...
def build_voice_files(source_basis_suffix: str) -> None:
    ...
    all_rows = [row for row in manifest.get("sources", []) if "matlock" in row.get("voice_slugs", [])]
    ...
    readme = f"""# Voice Record: Jack Matlock
...
| Source basis | `strategy-codex/statecraft/voices/matlock/matlock-index.md`{source_basis_suffix} |
...
"""
    ...
    source_index = f"""# Jack Matlock Source Index
